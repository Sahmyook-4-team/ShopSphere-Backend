...
public class ProductService {
    ...
    public ProductDTO.Response createProduct(String userId, ProductDTO.CreateRequest request) {
        ...
        Product product = Product.builder()
                ...
                // .imageUrl(request.getImageUrl()) // Product 엔티티에서 대표 이미지 필드를 제거하고
                // ProductImage로 통합 관리한다면 이 줄은 불필요
                .user(seller)
        ...
        // 예: request.getImageUrl() (대표 이미지), request.getAdditionalImageUrls()
        // (List<String> 추가 이미지)
        ...
        // 실제로는 ProductDTO.CreateRequest.getImages() 와 같이
        // List<ProductImageDTO.CreateRequest> 를 받는 것이 더 유연함
         ...
         * }
         */
        // savedProduct.setImages(productImages); // JPA 관계 설정에 따라 자동으로 업데이트되거나, 명시적으로
        // 설정
        // productRepository.save(savedProduct); // images 필드 업데이트를 위해 다시 저장 (선택적,
        // Cascade 설정에 따라 다름)
        ...
        Product finalProductWithImages = productRepository.findById(savedProduct.getId())
                .orElseThrow(
                        () -> new IllegalStateException("Product not found after save, id: " + savedProduct.getId()));
    ...
    private ProductDTO.Response convertToResponse(Product product) {
        ...
        // response.setImageUrl(product.getImageUrl()); // Product 엔티티에서 대표 이미지 필드를
        // 제거했다면 이 줄은 제거
        ...
        if (product.getImages() != null && !product.getImages().isEmpty()) {
            List<ProductImageDTO> imageDTOs = product.getImages().stream()
                    ...
                    .sorted(Comparator.comparing(ProductImageDTO::getDisplayOrder)) // 정렬
                    .collect(Collectors.toList());

            response.setImages(imageDTOs);
            response.setImageUrl(imageDTOs.get(0).getImageUrl());
        } else {
            response.setImages(new ArrayList<>());
            response.setImageUrl(null); // 이미지 없으면 null
        }
    ...
    private ProductImageDTO convertToProductImageDTO(ProductImage image) {
        ...
        // dto.setIsRepresentative(image.getIsRepresentative()); // isRepresentative 필드가
        // 있다면 매핑
        return dto;
    }
    ...
    private ProductOptionDTO.Response convertToOptionResponse(ProductOption option) {
        ...
        // dto.setOptionName(option.getOptionName()); // ProductOption 엔티티에 optionName
        // 필드가 있다면
    ...
    public ProductDTO.Response updateProduct(Integer productId, ProductDTO.UpdateRequest request) {
        ...
        // product.setImageUrl(request.getImageUrl()); // 대표 이미지 필드 관련
        ...
        // 2. 새로운 이미지 추가 처리
        // 3. 이미지 순서 변경 처리
        // ProductDTO.UpdateRequest에 이미지 관련 정보(삭제할 이미지 ID 목록, 추가할 이미지 URL/파일, 순서 정보 등)가
        // 필요
        // 예시: 삭제 로직
        ...
        if (request.getOptions() != null) {
            ...
            for (ProductOptionDTO.UpdateRequest optionRequest : request.getOptions()) {
                if (optionRequest.getId() != null) { // 기존 옵션 ID가 있는 경우
                    optionRepository.findById(optionRequest.getId())
                            .ifPresent(option -> {
                                if (optionRequest.getSize() != null)
                                    option.setSize(optionRequest.getSize());
                                if (optionRequest.getStockQuantity() != null)
                                    option.setStockQuantity(optionRequest.getStockQuantity());
                                if (optionRequest.getAdditionalPrice() != null)
                                    option.setAdditionalPrice(optionRequest.getAdditionalPrice());
        ...
        return convertToResponse(productRepository.findById(product.getId())
                .orElseThrow(
                        () -> new IllegalStateException("Product not found after update, id: " + product.getId())));
    }
    ...
    public List<ProductDTO.Response> searchProductsByKeyword(String keyword, String sortOption) { // sortOption 파라미터 추가
        ...
        if (!foundCategories.isEmpty()) {
            List<Integer> categoryIds = foundCategories.stream().map(ProductCategory::getId)
                    .collect(Collectors.toList());
        ...
        switch (sortOption) {
            case "sales_volume_desc": // 판매량 많은 순 (인기순으로 대체 가능)
                sortedProducts.sort(
                        Comparator.comparing(Product::getSalesVolume, Comparator.nullsLast(Comparator.reverseOrder()))
            ...
            case "price_desc": // 높은 가격순
                sortedProducts.sort(Comparator.comparing(Product::getPrice, Comparator.reverseOrder())
                        .thenComparing(Product::getId));
                break;
            ...
            default: // 기본 정렬 (예: ID 또는 이름순, 또는 판매량 많은 순)
                sortedProducts.sort(
                        Comparator.comparing(Product::getSalesVolume, Comparator.nullsLast(Comparator.reverseOrder()))
                                .thenComparing(Product::getId)); // 기본은 판매량순
                break;
        }
    ...
    public ProductDTO.Response convertToProductResponse(Product product) {
        ...
        if (product.getCategory() != null) {
            dto.setCategory(convertToCategoryResponse(product.getCategory())); // ProductCategory ->
